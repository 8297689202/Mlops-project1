...
from steps.hyperparameter import HyperparameterTuner
...
def main():
    ...
    search_results=tuner.run_tuning_workflow(data=train_data,search_type='random')
    ...
    if search_results:
        print(f"Best CV Score from Tuning: {search_results.best_score_:.4f}")
...
def train_with_mlflow():
    ...
    with mlflow.start_run() as run:
        ...
        if search_results:
            mlflow.log_metric("best_cv_score", search_results.best_score_)
            mlflow.log_params({f"best_{k}": v for k, v in search_results.best_params_.items()})
            mlflow.set_tag("hyperparameter_tuning", "completed")
            logging.info("Hyperparameter tuning completed successfully")
        else:
            mlflow.set_tag("hyperparameter_tuning", "failed")
            logging.warning("Hyperparameter tuning failed")
